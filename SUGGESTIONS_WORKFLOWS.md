...
- **etat_changed** – État changé (avec etat_from / etat_to)  
...
|---------------------|-------------|-------------|
| **fiche_archived** | Une fiche est archivée. | Notifier l’équipe, mettre à jour un reporting, déclencher un webhook externe. |
...
| **qualification_done** | Fiche qualifiée (passage à un état « qualifié » ou groupe donné). | Notification au commercial, création de tâche, envoi d’un SMS de rappel. |
| **signature_done** | Signature enregistrée (état signé / sous-état). | Notification, webhook vers facturation, message système. |
| **rdv_cancelled** | RDV annulé (état ou sous-état dédié). | SMS au client, notification commercial, réouverture de créneau. |
...
1. **rdv_cancelled** + **rdv_rescheduled** – Gestion des changements de RDV.  
...
| Réassignation après NRP | **etat_changed** (vers NRP) | **assign_confirmateur** (autre confirmateur) + **notification** |
