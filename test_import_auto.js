...
// Changer le répertoire de travail vers backend pour charger les dépendances
...
const DEFAULT_EMAIL = process.env.TEST_EMAIL || 'admin@example.com';
...
async function login(email, password) {
  try {
    ...
    logInfo(`Tentative de connexion avec: ${email}`);
...
function createAutoMapping(fileColumns) {
  ...
  const fieldMappings = {
    ...
    'prenom': ['prenom', 'firstname', 'first_name', 'prenom'],
  ...
  fileColumns.forEach(column => {
    ...
    for (const [dbField, variants] of Object.entries(fieldMappings)) {
      if (variants.some(v => colLower === v || colLower.includes(v) || v.includes(colLower))) {
        mapping[dbField] = column;
        logSuccess(`${dbField} <- ${column}`);
        break;
      }
    }
  });
