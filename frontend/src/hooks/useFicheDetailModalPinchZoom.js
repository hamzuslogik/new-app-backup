...
function applyZoomVars(contentEl, zoom) {
  if (!contentEl) return;
...
function resetZoomVars(contentEl) {
  if (!contentEl) return;
...
/**
 * Pinch 2 doigts sur le modal fiche (dashboard iOS) :
 * zoom in → texte plus grand, conteneur plus petit (marges conservées).
 * zoom out → l’inverse. Scroll vertical uniquement, pas de défilement horizontal.
 */
export function useFicheDetailModalPinchZoom(overlayRef, contentRef, enabled = false) {
  ...
  const applyZoom = useCallback((zoom) => {
    ...
    applyZoomVars(contentRef.current, zoomRef.current);
  }, [contentRef]);
