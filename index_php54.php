<?php
/**
 * Integration Vicidial -> CRM
 * VERSION COMPATIBLE PHP 5.4+
 *
 * Differences vs index.php (PHP 7+) :
 * - pas d'operateur ??
 * - polyfills hash_equals / json_last_error_msg
 * - mysqli sans mysqlnd (pas de get_result obligatoire)
 *
 * CONFIGURATION: voir vicidial_config.php
 */
...
if (!function_exists('json_last_error_msg')) {
    function json_last_error_msg() {
        ...
        $err = json_last_error();
        return isset($errors[$err]) ? $errors[$err] : 'Unknown error';
    }
}

/**
 * Fetch une ligne associative depuis un mysqli_stmt (avec ou sans mysqlnd).
 * @return array|null
 */
function mysqli_stmt_fetch_assoc_compat($stmt) {
...
$VICIDIAL_CONFIG = isset($CONFIG['vicidial']) ? $CONFIG['vicidial'] : array();
$CRM_CONFIG = isset($CONFIG['crm']) ? $CONFIG['crm'] : array();
$CACHE_CONFIG = isset($CONFIG['cache']) ? $CONFIG['cache'] : array(
    'enabled' => true,
    'duration' => 300,
...
function logConfiguredApiTokenInfo() {
    global $CRM_CONFIG;
    $token = (isset($CRM_CONFIG['api_token']) ? $CRM_CONFIG['api_token'] : '');
    if ($token === '') {
    ...
    $kind = (isset($payload['token_kind']) ? $payload['token_kind'] : null);
    $hasExp = isset($payload['exp']);
    $bypassIdle = !empty($payload['bypass_idle_timeout']);
    $userId = (isset($payload['userId']) ? $payload['userId'] : ((isset($payload['sub']) ? $payload['sub'] : '?')));
...
function extractApiErrorMessage($responseBody, $httpCode) {
    ...
    if (is_array($data)) {
        $msg = isset($data['message']) ? $data['message'] : (isset($data['error']) ? $data['error'] : null);
...
function connectVicidialDB() {
    ...
    $port = isset($VICIDIAL_CONFIG['port']) ? $VICIDIAL_CONFIG['port'] : 3306;
    $user = $VICIDIAL_CONFIG['user'];
    $password = $VICIDIAL_CONFIG['password'];
    $database = $VICIDIAL_CONFIG['database'];
    $charset = isset($VICIDIAL_CONFIG['charset']) ? $VICIDIAL_CONFIG['charset'] : 'utf8mb4';
    $serverName = isset($VICIDIAL_CONFIG['server_name']) ? $VICIDIAL_CONFIG['server_name'] : 'Vicidial';
...
function getUserByPseudo($pseudo) {
    ...
    if (isset($user['id'])) {
        writeLog("Utilisateur trouve: ID " . $user['id'] . " - Pseudo: " . ((isset($user['pseudo']) ? $user['pseudo'] : 'N/A')));
        return $user;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $ficheData = [
            'nom' => sanitizeInput((isset($_POST['nom']) ? $_POST['nom'] : '')),
            'prenom' => sanitizeInput((isset($_POST['prenom']) ? $_POST['prenom'] : '')),
            'tel' => sanitizeInput((isset($_POST['telephone']) ? $_POST['telephone'] : '')),
            'gsm1' => sanitizeInput((isset($_POST['gsm']) ? $_POST['gsm'] : '')),
            'adresse' => sanitizeInput((isset($_POST['adresse']) ? $_POST['adresse'] : '')),
            'cp' => sanitizeInput((isset($_POST['code_postal']) ? $_POST['code_postal'] : '')),
            'ville' => sanitizeInput((isset($_POST['ville']) ? $_POST['ville'] : '')),
        ];
        ...
        // Note PHP 5.4: empty() n'accepte pas d'appel de fonction (empty(trim(...)) interdit).
        $professionMrRaw = isset($_POST['profession_mr']) ? trim($_POST['profession_mr']) : '';
        $professionMadameRaw = isset($_POST['profession_madame']) ? trim($_POST['profession_madame']) : '';
        $professionMr = ($professionMrRaw !== '') ? sanitizeInput($_POST['profession_mr']) : null;
        ...
        $optionalFields = array(
            ...
            'details_etude' => (!empty($_POST['etude']) && strtoupper($_POST['etude']) === 'OUI' && $detailsEtudeRaw !== '')
        ...
        if (isset($responseData['success']) && $responseData['success'] && 
            ...
            $demandeId = (isset($responseData['data']['demandeId']) ? $responseData['data']['demandeId'] : null);
            $existingFicheId = (isset($responseData['data']['existingFicheId']) ? $responseData['data']['existingFicheId'] : null);
            $message = "Fiche existante, une demande d'insertion a ete envoyee au backoffice.";
            writeLog("DEMANDE D'INSERTION CREE: " . $message . " (demandeId=" . ((isset($demandeId) ? $demandeId : 'N/A')) . ", existingFicheId=" . ((isset($existingFicheId) ? $existingFicheId : 'N/A')) . ")");
            // Rediriger pour éviter la resoumission - stocker comme notice (rouge)
            $_SESSION['notice_message'] = $message;
            header('Location: ' . $_SERVER['PHP_SELF'] . '?reload=1');
            exit;
            ...
            // Erreur
            $errorMsg = (isset($responseData['message']) ? $responseData['message'] : "Erreur HTTP $httpCode");
    ...
    <div class="container">
        ...
        <?php if ($vicidialData): ?>
            <div class="vicidial-info">
                ...
                Dernière modif: <?php echo htmlspecialchars((isset($vicidialData['modify_date']) ? $vicidialData['modify_date'] : 'N/A')); ?>
            </div>
        <?php endif; ?>
        ...
        <form method="POST" action="">
            ...
            <div class="form-section">
                ...
                <div class="form-grid">
                    <div class="form-group">
                        <label>Nom</label>
                        <input type="text" name="nom" value="<?php echo htmlspecialchars(isset($vicidialData['last_name']) ? $vicidialData['last_name'] : ''); ?>">
                    </div>
                    <div class="form-group">
                        <label>Prénom</label>
                        <input type="text" name="prenom" value="<?php echo htmlspecialchars(isset($vicidialData['first_name']) ? $vicidialData['first_name'] : ''); ?>">
                    </div>
                    <div class="form-group">
                        <label>Téléphone</label>
                        <input type="tel" name="telephone" value="<?php echo htmlspecialchars(isset($vicidialData['phone_number']) ? $vicidialData['phone_number'] : ''); ?>">
                    </div>
                    <div class="form-group">
                        <label>GSM</label>
                        <input type="tel" name="gsm" value="<?php echo htmlspecialchars(isset($vicidialData['alt_phone']) ? $vicidialData['alt_phone'] : ''); ?>">
                    </div>
                    <div class="form-group full-width">
                        <label>Adresse</label>
                        <input type="text" name="adresse" value="<?php 
                            $adresse = trim((isset($vicidialData['address1']) ? $vicidialData['address1'] : '') . ' ' . (isset($vicidialData['address2']) ? $vicidialData['address2'] : ''));
                            echo htmlspecialchars($adresse);
                        ?>">
                    </div>
                    <div class="form-group">
                        <label>Code Postal</label>
                        <input type="text" name="code_postal" value="<?php echo htmlspecialchars(isset($vicidialData['postal_code']) ? $vicidialData['postal_code'] : ''); ?>">
                    </div>
                    <div class="form-group">
                        <label>Ville</label>
                        <input type="text" name="ville" value="<?php echo htmlspecialchars(isset($vicidialData['city']) ? $vicidialData['city'] : ''); ?>">
                    </div>
                </div>
            </div>
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="datetime-local" name="date_appel" value="<?php 
                            $modifyDate = isset($vicidialData['modify_date']) ? $vicidialData['modify_date'] : null;
                        ...
                        <select name="produit" id="produit" onchange="toggleProductFields()">
                            ...
                            <?php foreach ($produits as $produit): ?>
                                <option value="<?php echo htmlspecialchars($produit['id']); ?>" 
                                        data-nom="<?php echo htmlspecialchars(strtoupper(isset($produit['nom']) ? $produit['nom'] : '')); ?>">
...
<?php
// Fin du script
$totalTime = round((microtime(true) - (isset($_SERVER['REQUEST_TIME_FLOAT']) ? $_SERVER['REQUEST_TIME_FLOAT'] : microtime(true))) * 1000, 2);
