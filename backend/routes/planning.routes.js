...
// Fiches confirmées (état 7 = CONFIRMER) dont la date RDV est le jour choisi.
// - Si date >= aujourd'hui : source = fiches (date_rdv_time, id_etat_final=7)
// - Si date < aujourd'hui : source = fiches_histo (historique confirmations, id_etat=7, date_rdv_time)
// - jour : toutes ces fiches du jour
// - affilie : celles attribuées à un commercial
// - non_affilie : celles non attribuées
// =====================================================
router.get('/rdv-vue', authenticate, async (req, res) => {
  try {
    ...
    // Log entrée
    console.log('[rdv-vue] Requête:', { type, date: d, isPastDate, query: req.query });

    let rows = [];

    if (isPastDate) {
      // Date passée : chercher les RDV via fiches_histo (historique des confirmations, id_etat=7)
      ...
      const histoWhereOuter = 'WHERE ' + histoWhereParts.join(' AND ');
      console.log('[rdv-vue] Source: fiches_histo (date passée). WHERE:', histoWhereOuter);
      ...
      rows = await query(
        `SELECT 
          ...
          'CONFIRMER' as etat_titre
        FROM (
          SELECT id_fiche, MAX(id) as max_id
          FROM fiches_histo
          WHERE id_etat = 7 AND date_rdv_time >= ? AND date_rdv_time <= ?
          GROUP BY id_fiche
        ) last
      ...
      // Date aujourd'hui ou future : source = fiches (id_etat_final=7, date_rdv_time du jour)
      const conditions = [
      ...
      console.log('[rdv-vue] Source: fiches. WHERE:', whereClause);
